...
import {
  Mail, Phone, MapPin, Send, MessageSquare, Clock, Sparkles , Users, Zap, Shield, ChevronDown
} from 'lucide-react';
...
export default function Contact() {
  ...
  // ساعات العمل: من 6 صباحًا إلى 9 مساءً
  ...
  const subjects = [
    ...
    { value: 'ecommerce', label: 'متاجر إلكترونية', icon: '🛒', color: '#8b5cf6' },
        ...
        <motion.section variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mb-24">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    {['name', 'email', 'phone'].map((field) => (
                      ...
                      >
                        <input
                          ...
                          placeholder={`أدخل ${field === 'name' ? 'اسمك ...' : field === 'email' ? 'البريد الالكتروني ' : 'الهاتف '}`}
            ...
            <motion.div variants={itemVariants} className="space-y-6">
              ...
              <motion.div
                variants={itemVariants}
                className={`backdrop-blur-sm rounded-2xl border p-6 text-center transition-all
                  ${isWorkingHours
              ...
              >
                ...
                <motion.div
                  ...
                  className={`w-4 h-4 rounded-full mx-auto mb-4
                    ${isWorkingHours ? "bg-emerald-400" : "bg-red-500"}
                  `}
                />
                <h3 className="text-xl font-bold text-white mb-2">
                  {isWorkingHours ? "متاحون الآن" : "خارج أوقات العمل"}
                </h3>
                <p className={`font-semibold
                  ${isWorkingHours ? "text-emerald-400" : "text-red-400"}
                `}>
                  {isWorkingHours
                    ? "فريق الدعم جاهز للرد فوراً!"
                    : "ساعات العمل من 6 صباحًا إلى 9 مساءً"}
                </p>
              </motion.div>
            </motion.div>
